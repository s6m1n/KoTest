...
class FlowTest {
    ...
    @Test
    fun `flow로 값을 보내고 받을 수 있다`() = runTest {
        val numbers = flow {
            for (i in 1..5) {
                emit(i)
            }
    ...
    @Test
    fun `flow의 중간 연산자를 체이닝할 수 있다`() = runTest {
        val numbers = flowOf(1, 2, 3, 4, 5)
            .map { it * 2 }
            .filter { it > 5 }
    ...
    @Test
    fun `flow를 중단 없이 collect할 수 있다`() = runTest {
        val numbers = flow {
            for (i in 1..3) {
                delay(100)
                emit(i)
    ...
    @Test
    fun `flow를 처음부터 일부만 받을 수 있다`() = runTest {
        val numbers = flowOf(1, 2, 3, 4, 5)
            .take(3)
